...
class TestCoreCollapsedHalo(object):
    ...
    def test_alphar(self):
        ...
        args = {'gamma_inner': gamma_inner,
                ...
                'rt_kpc': rt_kpc,
                'm_target_r200': m_target_r200,
                'm_target_R': m_target_R,
                'r_match_kpc': r_match_kpc,
                'Rs_inner_kpc': Rs_inner_kpc}
        ...
        plaw_halo = CoreCollapsedHalo(mass, x, y, r3d, z,
                                         sub_flag, lens_cosmo, args, truncation_class, concentration_class, unique_tag)
        bh_halo = CoreCollapsedHaloBH(mass, x, y, r3d, z,
                                      sub_flag, lens_cosmo, args, truncation_class, concentration_class, unique_tag)

        x = np.logspace(-1.0, 1.5, 100)
        r = x * tnfw_subhalo.nfw_params[1]
        kpc_per_arcsec = 6.2
        r_arcsec = r / kpc_per_arcsec
        alphar_plaw = plaw_halo.deflection_angle(r_arcsec)
        alphar_bh = bh_halo.deflection_angle(r_arcsec)
        npt.assert_equal(len(alphar_plaw) == len(alphar_bh), True)
        #
        # import matplotlib.pyplot as plt
        #
        # plt.loglog(r_arcsec, alphar_plaw)
        # plt.loglog(r_arcsec, alphar_bh)
        # plt.show()
