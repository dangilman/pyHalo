...
class Realization(object):
    ...
    def filter(self, aperture_radius,
        ...
        if len(check_distances) > 0:
            ...
            # Halo positions: shape (N_check, 1) for broadcasting against (N_check, N_rays)
            dx = self.x[check_distances, None] - ray_x
        ...
        halos = []
        keep_inds_global = np.where(keep_inds)[0]
        for zi in self.unique_redshifts:
            plane_halos, _ = self.halos_at_z(zi)
            inds_at_z = np.where(self.redshifts == zi)[0]
            keep_at_z = np.intersect1d(inds_at_z, keep_inds_global)
            inds_keep_at_plane = np.searchsorted(inds_at_z, keep_at_z)
            for i in inds_keep_at_plane:
